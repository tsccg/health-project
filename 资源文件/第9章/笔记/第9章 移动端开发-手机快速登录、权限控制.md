...
为获取验证码按钮绑定事件，并在事件对应的处理函数中校验手机号，如果手机号输入正确则显示30秒倒计时效果并发送ajax请求，发送短信验证码。**已有，看一眼即可。**
...
认证：系统提供的用于识别用户身份的功能，通常提供用户名和密码进行登录其实就是在进行认证，认证的目的是让系统知道你是谁。**1张用户表**

授权：用户认证成功后，需要为用户授权，其实就是指定当前用户可以操作哪些功能。 **5张表**
...
![image-20200528145930730](img/image-20200528145930730.png)
...
### 3.4 Spring Security入门案例-重点

#### 3.4.1 工程搭建

创建maven工程springsecuritydemo，打包方式为war，为了方便起见我们可以让入门案例工程依赖health_interface，这样相关的依赖都继承过来了。
...
#### 3.4.3 配置spring-security.xml-重点理解
...
测试：访问http://localhost:85/，马上跳转至登陆页面

![image-20200525183316221](img/image-20200525183316221.png)

输入admin admin 。再访问http://localhost:85/

![image-20200525183541193](img/image-20200525183541193.png)
...
第三步：修改spring-security.xml文件，加入表单登录信息的配置。位置：security:http中
...
测试：访问http://localhost:85/ 输入 admin admin

![image-20200525190519556](img/image-20200525190519556.png)

登陆成功

![image-20200525190535889](img/image-20200525190535889.png)

去掉<security:csrf 配置，再登陆时访问框架带的登陆页面。

#### 3.5.3 从数据库查询用户信息-自己写代码！
...
a       (+1)        b           (-1)     a
1.2非对称性加密
anth.war 私钥       ------------   user.war 公钥
123456  ---- 私钥 ---- abcdef  ------公钥   可用
RSA linux免密登陆 ssh 192.168.1.1

2不可逆加密
123456----->adsfdsafaasdfsd                md5
```
...
<bean id="userService2" class="com.itheima.security.UserService2"></bean>
...
通过上面的配置可以发现，如果用户要退出登录，只需要请求http://localhost:85/logout.do这个URL地址就可以，同时会将当前session失效，最后页面会跳转到login.html页面。
...
![image-20200528172231462](img/image-20200528172231462.png)
...
​	3入门 ：web   web.xml spirng.xml    ------>安全指定规则     提供资源

​	4改进

​			1 security=none

​			2 登陆页面：1login.html 2 security=none 3规则 4csrf关掉
...
​			6方法级别授权   @PreAuthorize("hasAuthority('add')")
