...
检查组对应的实体类为CheckGroup，对应的数据表为**t_checkgroup**。检查组和检查项为多对多关系，所以需要中间表**t_checkgroup_checkitem**进行关联。
...
![image-20200416233004672](img/image-20200416233004672.png)

![image-20200416233024418](img/image-20200416233024418.png)
...
![image-20200416234516333](img/image-20200416234516333.png)

## 4. 编辑检查组

### 4.1 完善页面

用户点击编辑按钮时，需要弹出编辑窗口并且将当前记录的数据进行回显，用户修改完成后点击确定按钮将修改后的数据提交到后台进行数据库操作。此处进行数据回显的时候，除了需要检查组基本信息的回显之外，还需要**回显当前检查组包含的检查项**（以复选框勾选的形式回显）。
...
![image-20200521084905940](img/image-20200521084905940.png)



1

2遇到困难，不要怕，战胜恐惧，加油，奥利给！

​	项目一 业务逻辑。

​	项目二 spirng boot  cloud.业务逻辑。
